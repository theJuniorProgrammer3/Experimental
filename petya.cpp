...
void purchase() {
	...
	int h, w;
	getmaxyx(stdscr, h, w);
	printw("You became victim of the PETYA RANSOMWARE!\n");
	for(int i = 0; i < w; i++){
		printw("█");
	}
	printw("\n");
	...
	printw("http://petya37h5tbhyuki.onion/7FdkQs\n");
	printw("http://petya5koahtsf7sv.onion/7FdkQs\n");
	printw("3. Enter your personal decryption code there:\n");
	printw("826S81M-CMrGLC-ST2vmJ-SLKuv6-FydUyS-JuiFf1-e7zKFB-x6j8iS-cFooyJ-kCWRSD-SCs1zF-Xezz-prksv-y5uU3D-JQpZ1N\n");
	printw("If you already purchased your key, please enter it below.\n");
	//std::string key = "PLEASE_QUIT";
	char input[100];
	printw("Key: ");
	getstr(input);	
